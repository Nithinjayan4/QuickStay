    ...
    <div className='py-28 md:py-35 px-4 md:px-16 lg:px-24 xl:px-32'>
        ...
        <div className='flex flex-col lg:flex-row mt-6 gap-6'>
            ...
            <div className='grid grid-cols-2 gap-4 lg:w-1/2 w-full'>
                {room?.images.length > 1 && room.images.map((image, index) => (
                    ...
                    src={image} alt="RoomImage" key={index} className={`w-full rounded-xl shadow-md object-cover cursor-pointer ${mainImage === image && 'outline-3 outline-orange-600'}`} />
                ))}
            </div>
        </div>
        {/* Room Highlights */}
        ...
        <div className='max-w-3xl border-y border-gray-300 my-15 py-10 text-gray-500'>
            <p >Guests will be allocated on the ground floor according to availability.
                You get a comfortable Two bedroom apartment has true city feeling. The 
                price quoted is for two guests, at the guests slot please mark the number of guests
                to get exact price for groups.The Guests will be allocated ground floor according to availability.
